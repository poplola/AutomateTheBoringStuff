...
def main():
    ...
    FONTS_FOLDER = '/Users/flaviolago/Documents/Work/Training/Python/MSCode Projects/TestFile_L/19'
    ...
    for guest in guests:
        ...
        print(f"{FLOWER_FILENAME} size : {deco.size}")
    ...
    #=====
        # font = ImageFont.truetype(os.path.join(FONTS_FOLDER, '/liberation/LiberationSerif-Regular.ttf'), 115)
        font = ImageFont.truetype(os.path.join(FONTS_FOLDER, 'WESTATE_.TTF'), 115)
    #=====
        # draw.text((int((deco_width - text_width) / 2), int((deco_height - text_height) / 2)), guest, fill='black', font=font)
        draw.text((int((deco_width - text_width) / 7), int((deco_height - text_height)  / 2)), guest, fill='black', font=font)
